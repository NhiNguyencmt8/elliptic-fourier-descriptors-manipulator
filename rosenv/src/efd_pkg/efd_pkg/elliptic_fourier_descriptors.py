...
def sort_contours(contours):
    ...
    #array of points, at 0th row there will be the upper-left mostly point of the 0th contour of list contours, and so on
    points_per_contour = np.zeros((len(contours),2), dtype = int)

    #retrieving the upper-left mostly point of the contours
    counter = 0
    for c in contours:
        cres = np.reshape(c,(-1,2))
        idx = np.lexsort( (cres[:,0],cres[:,1]) )
        sorted_contours = cres[idx,:]
        points_per_contour[counter,:] = sorted_contours[0,:]
        counter = counter + 1

    #sort the contours so that the scan order is raster
    sorted_idx = np.lexsort((points_per_contour[:,0], points_per_contour[:,1]))
    sorted_contours = []
    for idx in sorted_idx:
        sorted_contours.append(contours[idx])

    #voilla..wonderful raster sorted contours
...
def elliptic_fourier_descriptors(bin_im,N):
